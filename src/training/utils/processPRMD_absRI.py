...
# Perform the matrix multiplication with B and the transpose of B
				temp_result = np.matmul(B, B.T)
				result.append(temp_result)

# Stack the result to get an array of shape Mx22x22
			result = np.stack(result, axis=0)
# Extract movement, subject and episode number from filename
			movement, _, _, _ = filename.split('_')
			print(filename, "1", flush=True)
			prmdDataset.append((result, movement, 1))
...
# Extract movement, subject and episode number from filename
			...
			prmdDataset.append((result, movement, 0))
	...
	# Save to pickle
	with open(cleanPath + 'prmdDataset_absRI.pkl', 'wb') as f:
